...
export default function UsersPage() {
  ...
  return <div>
    <PageHeader eyebrow="Access / members" title="用户管理" description="管理组织成员、账号状态与角色归属。变更会实时影响下次访问鉴权。" action={<Button onClick={openCreate}><Plus className="size-4" />新建用户</Button>} />
    {message && <StatusMessage message={message} />}{error && <StatusMessage message={error} tone="error" />}
    <section className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-2"><UsersIcon className="size-4 text-muted-foreground" /><span className="text-sm font-semibold text-foreground">全部用户</span><Badge variant="muted">{filtered.length}</Badge></div><SearchBar value={query} onChange={(value) => { setQuery(value); setPage(1); }} placeholder="搜索姓名或邮箱..." /></div>
      {selected.length > 0 && <div className="flex items-center justify-between border-b border-primary/20 bg-primary/10 px-4 py-2.5"><span className="text-sm font-medium text-primary">已选择 {selected.length} 个用户</span><Button variant="outline" size="sm" onClick={bulkDisable}><UserMinus className="size-4" />批量停用</Button></div>}
      <Table><TableHeader><TableRow><TableHead className="w-12"><Checkbox checked={allVisibleSelected} onCheckedChange={(checked) => toggleAll(checked === true)} aria-label="选择当前页全部用户" /></TableHead><TableHead>成员</TableHead><TableHead>角色</TableHead><TableHead>状态</TableHead><TableHead>加入时间</TableHead><TableHead className="w-20 text-right">操作</TableHead></TableRow></TableHeader><TableBody>{pageItems.map((user) => <TableRow key={user.id} data-state={selected.includes(user.id) ? "selected" : undefined}><TableCell><Checkbox checked={selected.includes(user.id)} onCheckedChange={(checked) => toggleOne(user.id, checked === true)} aria-label={`选择${user.name}`} /></TableCell><TableCell><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{user.name.slice(0, 2)}</div><div><p className="font-medium text-foreground">{user.name}</p><p className="text-xs text-muted-foreground">{user.email}</p></div></div></TableCell><TableCell><div className="flex flex-wrap gap-1.5">{user.roles.map(({ role }) => <Badge key={role.id} variant="muted">{role.name}</Badge>)}</div></TableCell><TableCell><Badge variant={user.status === "ACTIVE" ? "success" : "danger"}>{user.status === "ACTIVE" ? "正常" : "已停用"}</Badge></TableCell><TableCell className="text-muted-foreground">{dateFormatter.format(new Date(user.createdAt))}</TableCell><TableCell><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" onClick={() => openEdit(user)} aria-label={`编辑${user.name}`} title="编辑"><Edit3 className="size-4" /></Button><Button size="icon" variant="ghost" aria-label="更多操作" title="更多操作"><MoreHorizontal className="size-4" /></Button></div></TableCell></TableRow>)}</TableBody></Table>
